...
def COT_prompts(uri,prompts):
    result = [] 
    result.append([prompts.pop(0), uri]) 
...
def chain_of_though(model_name, uri, prompts, system_instruction = None): 
    ...
    if system_instruction: 
        ...
        model = genai.GenerativeModel(model_name=model_name).start_chat()
...
system_instr = """You are ultimately tasked to give output that should provide a detailed, step-by-step guide on how to test each functionality. The guide should be divided into the following sections:
